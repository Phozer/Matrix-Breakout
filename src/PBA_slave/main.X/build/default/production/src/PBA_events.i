...
static timeevent_t timeEvents[10] = {0};
...
void EVENTS_Update(void)
{
    ...
    for(i=0;i<8;i++)
    {
        if((((switchValue) & (1 << (i)))>0))
        {
            if(switchCount[i]<255)
            {
                switchCount[i]++;
                if(switchCount[i]>=oneSec)
                {
                    p_toEvents->valuePressed|=((uint16_t)1<<(8+i));
                }

                else
                {
                    if(switchCount[i]>=o_5Sec)
                    {
                        p_toEvents->valuePressed|=((uint16_t)1<<i);
                    }
                }
            }
        }
        ...
        {
            switchCount[i]=0;
            p_toEvents->valuePressed&=~(((uint16_t)1<<i)|((uint16_t)1<<(8+i)));
        }
...
static uint8_t EVENTS_TimeoutHandler(uint16_t timeoutMS)
{
    ...
    else
    {
        ...
        for(i=0; i<10 && timeEvents[i].ms!=timeoutMS; i++);
        if(i==10)
        {

            for(i=0; i<10 && timeEvents[i].active>0; i++);
            if(i==10) return 0xFF;
        ...
        {
            if(timeEvents[i].active && timeEvents[i].eventOccured)
            {
...
static uint8_t EVENTS_ActiveUntilHandler(uint16_t activeUntilMS)
{
    ...
    for(i=0; i<10 && timeEvents[i].ms != activeUntilMS; i++);
    if(i==10)
    {
...
{
    uint8_t i;
    for(i=0; (i<10) && (timeEvents[i].ms != t); i++);
     if( i == 10)
     {
         return 1;
     }
